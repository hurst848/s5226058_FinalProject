...
public class Hurst_PlanetGenerator : MonoBehaviour
{
    ...
    {           
        ...
        //! Add emtpy vertex buffer to the shader
            var vertexDataBuffer = new ComputeBuffer((BaseResolution * BaseResolution) + (25 * 25 * 4), 3 * sizeof(float));
            vertexGenerationShader.SetBuffer(0, "Verticies", vertexDataBuffer);

        //! Add empty triangle buffer to the shader
            var triangleDataBuffer = new ComputeBuffer((((BaseResolution - 1) * (BaseResolution - 1)) * 6) + (((24) * (24)) * 6 * 4), sizeof(int));
        ...
        //! Retrieve the Vertices from the completed shader
            Vector3[] verticies = new Vector3[(BaseResolution * BaseResolution) + (25 * 25 * 4)];
            vertexDataBuffer.GetData(verticies);

        //! Retrieve the Triangles from the completed shader
            int[] triangles = new int[(((BaseResolution - 1) * (BaseResolution - 1)) * 6) + (((24) * (24)) * 6 * 4)];
            triangleDataBuffer.GetData(triangles);

        //! Retrive colours and map them to the correct gradient
            VertexColour[] biomeColours = new VertexColour[(BaseResolution * BaseResolution) + (25 * 25 * 4)];
            biomeColourDataBuffer.GetData(biomeColours);
            Color[] vertColours = new Color[(BaseResolution * BaseResolution) + (25 * 25 * 4)];
