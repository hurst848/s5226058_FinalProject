...
namespace HGE
{
	...
	std::shared_ptr<Mesh> MeshBuilder::CreateMesh(std::string _name, std::vector<vec3> _verticies, std::vector<vec3> _normals, std::vector<int> _indicies)
	{
		std::shared_ptr<Mesh> generatedMesh = std::make_shared<Mesh>();
		...
		{
			dataBuffer_vertex.push_back(_verticies.at(_indicies.at(i)).x);
			dataBuffer_vertex.push_back(_verticies.at(_indicies.at(i)).y);
			dataBuffer_vertex.push_back(_verticies.at(_indicies.at(i)).z);
		

			dataBuffer_normal.push_back(_normals.at(_indicies.at(i)).x);
			dataBuffer_normal.push_back(_normals.at(_indicies.at(i)).y);
			dataBuffer_normal.push_back(_normals.at(_indicies.at(i)).z);
		...
		generatedMesh->VertexCount = _indicies.size();

		generatedMesh->Name = _name;
